...
import {
  evaluateGuidedWritingParagraph,
  getLatestGuidedWritingParagraphDraft,
} from "../../../../lib/guided-writing-paragraph-service";
import { GuidedWritingRequestError } from "../../../../lib/guided-writing-service";

const submitSchema = z.object({
  sessionId: z.string().uuid(),
  draftId: z.string().uuid(),
  draftText: z.string().min(1).max(5_000),
}).strict();
...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json({ draft: getLatestGuidedWritingParagraphDraft(connection, sessionId) });
  } catch (error) {
    if (error instanceof GuidedWritingRequestError) {
      return NextResponse.json({ error: error.message, code: error.code }, { status: error.statusCode });
    }
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to load paragraph draft" }, { status: 500 });
  } finally {
    connection.close();
  }
}
...
export async function POST(request: Request) {
  ...
  const connection = await openAppDatabase(request);
  try {
    const result = await evaluateGuidedWritingParagraph({ connection, ...parsed.data });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to check this paragraph" }, { status: 500 });
  } finally {
    connection.close();
  }
}
